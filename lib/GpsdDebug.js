...
var Debug = function(level, format) {  //+ arguments
    ...
    if (this.debug >= level) {
        ...
        if (this.debug >5) console.log ("-%d- %s/%s:%d [%s] -- %j", level, trace.file, trace.name, trace.line, this.uuid, this.message);
           else console.log ("-- %s [%s] -- %j", level, trace.name, this.uuid, this.message);
    };
};
