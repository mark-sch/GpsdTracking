...
GpsdHttpClient.prototype.Distance = function (lat1, lon1, lat2, lon2) {
...
GpsdHttpClient.prototype.ProcessData = function(data) {
    ...
    switch (data.cmd) {
        ...
        case "TRACKER" : 
            ...
            // this should never happen, but if protocol is hacked by device !!!
            if (gpsd.activeClients [this.imei] !== undefined) return (-1);
            ...
            if (update) { // update device stat in sever ram and database
                // even if user not present in DB we keep a minimal set of info in RAM
                this.count ++; 
