 ...
 * sog and tic. Sends nmea paquets at your selected tic rate. Stop at file end.
 * 
 * syntax:  handle= new GpsdSimulator(opt);
 * 
 * opts={ file: xxxx GPX file exported from OpenCPN or other GPX compliant tools
 *  sog: xxx --tic=xxx --hostname=xxx --srvmod --port=xxx ]
...
GpsdSimulator = function(opts) {
    ...
    switch (this.opts.proto) {
        ...
        case 'gps103' : this.opts.mod103 =true;  break;
        default:
    ...
    this.Debug (7, "Main Options: gpxfile=%s sog=%d tic=%d proto=%s hostname=%s port:%d shipname=%s srvmod=%s, cltmod=%s", this.opts.gpxfile
               , this.opts.sog, this.opts.tic, this.opts.proto, this.opts.host, this.opts.port, this.opts.shipname, this.opts.srvmod, this.opts.cltmod);
...
GpsdSimulator.prototype.ProcessWaypts  = function (job) {
    ...
    var NmeaPaquet = function (simulator) {
        ...
        nowdate=NmeaDate();
        paquet=util.format ("$GPRMC,%s.00,A,%s,%s,%s,%s,%s,%s,%s,,,A", nowdate[1],Dec2Min(job.lat),lato, Dec2Min(job.lon), lono, job.sog, job.cog, nowdate[0]);
    ...
    this.Debug (4, "--%d-- lat=%s,lon=%s sog=%s cog=%s",fixCount++, job.lat.toFixed(4),job.lon.toFixed(4),job.sog.toFixed(1),job.cog.toFixed(1));
