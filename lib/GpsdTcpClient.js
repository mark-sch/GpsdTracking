...
GpsdTcpClient= function (socket) {
    ...
    this.debug     = this.controler.debug; // inherit controler debug level
    this.adapter   = this.controler.adapter;
    this.socket    = socket; 
    ...
    switch (data.cmd) {
        ...
        case "TRACKER" : 
            ...
            if (this.stamp !== undefined) {
                //console.log ("**** pos1= %s,%s Stamp=%s,%s", data.lat, data.lon, this.stamp.lon, this.stamp.lat);
