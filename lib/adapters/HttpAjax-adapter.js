...
DevAdapter = function(controler) {
    ...
    this.control   = 'http';
    this.cors      = controler.svcopts.cors || false;
    
    // define root dir for html pages.
    if (controler.svcopts.rootdir !== undefined)  this.rootdir= controler.svcopts.rootdir;
    else  this.rootdir= path.join (__dirname, "../../www");
    
    this.Debug (1,"New Httpd: %s rootdir=%s port=%s", this.uid, this.rootdir, controler.svcopts.port);    
};
...
DevAdapter.prototype.QueryDevTrack = function(question, request, response) {
    ...
    var srchost = request.headers.host;
    var query   = question.query;
    var device  = gpsd.activeClients [query.devid];
    ...
    DBcallback = function(dbresult) {
        ...
        for (var idx in dbresult) {
            ...
            var ptsinfo=util.format ("%s<br>Position:[%s,%s] Speed:%s",pos.date,pos.lon.toFixed(4),pos.lat.toFixed(4),pos.speed.toFixed(1));
    ...
    gpsd.backend.LookupDev (DBcallback, query.devid, parseInt(query.llist)||10);
};
...
DevAdapter.prototype.ProcessRestApi = function(question, request, response) {
    ...
    if (parseInt (query['key']) !== 123456789) {
...
DevAdapter.prototype.ProcessFile = function(question, request, response, addvarmod) {
    ...
    function ReadFileCB (err, byteread, buffer) {
        if (err) {
            ...
            if (addvarmod) response.write ("<script>location['GPSD_HTTP_AJAX']=true</script>");
...
DevAdapter.prototype.ProcessData = function(request, response) {
    ...
    response.setHeader("Server", "GpsdTracking-HttpAjax");
    // provide a default search to index.html
    if (question.pathname === '/') question.pathname = "/index.html";
    ...
    case ".html":
        ...
        response.setHeader("Content-Type", "text/html");
        this.ProcessFile (question,request,response, true);
        break;
    case '.js':
        response.setHeader("Content-Type", "text/javascript"); 
        this.ProcessFile (question,request,response, false);
        break;
    case '.css':
