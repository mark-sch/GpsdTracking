    ...
    switch (data.cmd) {
        ...
        case 'DBSEAR':
            ...
            var DBcallback = function (dbresult) {
                ...
                for (var idx = 0; (idx < dbresult.length); idx ++) {
                    ...
                    posi.lon   = (parseInt(posi.lon   * 10000))/10000;
                    posi.lat   = (parseInt(posi.lat   * 10000))/10000;
                    posi.speed = (parseInt(posi.speed * 10))/10;
                    posi.crs   = (parseInt(posi.crs   * 10))/10;
        ...
        case "DEVINFO":  // print info avaliable from gpsd activeClient array
            try {
                ...
                var elapse= parseInt((new Date().getTime()- dev.lastshow)/1000);
                dev.lon   = (parseInt(dev.lon * 10000))/10000;
                dev.lat   = (parseInt(dev.lat * 10000))/10000;
                dev.speed = (parseInt(dev.speed * 10))/10;
                dev.crs   = (parseInt(dev.crs * 10))/10;
                var info= util.format ("> -%d- imei/mmsi= %s Name= '%s' LastShow: %ss Adapter: %s\n"
                              , count, devId, dev.name, elapse, dev.adapter.info);
                socket.write (info);
                info=util.format (">    Lat:%s Lon:%s Speed:%s Alt:%s Crs:%s Time:%s\n"
                             , dev.stamp.lat, dev.stamp.lon, dev.stamp.speed, dev.stamp.alt, dev.stamp.crs, dev.stamp.date.toJSON());
                 socket.write (info);
            } catch(err) {
                socket.write ("> - imei: " + data.imei + "No Stamp Info [try dev track]\n");
            }
            break;
