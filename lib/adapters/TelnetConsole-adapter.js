...
DevAdapter = function(controler) {
    ...
    this.uid      = "adapter:telnet//" +  controler.svcopts.port;
    ...
    switch (data.cmd) {
        ...
        case "DEVLIST": // list devices from gpsd active list
            ...
            for (var devId in gpsd.activeClients) {
                var dev= gpsd.activeClients[devId];
                if (dev.loged) {
                    count ++;
            ...
            try {
              ...
              var DBcallback = function (dbresult) {
                ...
                for (var idx = 0; (idx < dbresult.length); idx ++) {
                    ...
                    , idx++, posi.lat, posi.lon, posi.sog, posi.alt, posi.cog, posi.date.toJSON());
                    socket.write (info);
                 }
              };
