...
DevAdapter = function(controler) {
    ...
    "bnf": { // WARNING: only one space in between TOKEN ex: "STOP EOF"
        'data': [["EOL"        ,"this.cmd='EMPTY'; return (this);"]
                ...
                ,["BASE SEAR TXT EOL" ,"this.cmd='DBSEAR' ; this.imei=$3;return (this);"]
...
DevAdapter.prototype.ParseData = function(device, socket, buffer) {
    ...
    switch (data.cmd) {
        ...
        case "HELP":   // better than no documentation :)
            ...
            socket.write (">   db search xxxx                    [search last devices positions in database imei=xxx]\n");
            socket.write (">\n");
            ...
            socket.write (">   evt                               [register a listener to receive event from gpsd as user application does\n");
        ...
        case "DEVLIST": // list devices from gpsd active list
            ...
            for (var devId in gpsd.activeClients) {
                ...
                if (dev.loged) {
                    count ++;
                    socket.write ("> - " + count + " - imei: " + devId + " Name:[" + dev.name + "] Adapter=" + dev.adapter.info +"\n");
                }
            }
        ...
        case "DEVALL": // list devices from gpsd active list
            ...
            count ++;
                dev= gpsd.activeClients[devId];
                socket.write ("> - " + count + " - imei: " + devId + " Name:[" + dev.name + "] Loged:" + dev.loged + " Adapter=" + dev.adapter.info +"\n");
            }
        ...
        case "DEVINFO":  // print info avaliable from gpsd activeClient array
            try {
                ...
                var date  = new Date (dev.timestamp);
                socket.write ("> \nDev="  + dev.uid + " Adpt=" + dev.adapter.uid + "Ctrl=" + dev.adapter.controler.uid +"\n");
                socket.write ("> Imei: " + data.imei + " Name:[" + dev.name + "] Loged:" + dev.loged + " Adapter:" + dev.adapter.info +"\n");
                socket.write ("> Lat: " + dev.latstamp + " Lon: " + dev.lonstamp + " Speed:" + dev.speedstamp + " TimeStamp:" + date +"\n\n");
            } catch(err) {
                socket.write ("> - imei: " + data.imei + "No position info in active list\n");
            }
            break;
