...
DevAdapter = function(controler) {
    ...
    "bnf": { // WARNING: only one space in between TOKEN ex: "STOP EOF"
        ...
        , 'GPSDATA' : [ // F,215931.000,A,4737.1058,N,00245.6524,W,0.00,0  [multiple end option W,0.00,0; | W,0.00,; | W,0.00;]
                ["FULL FLOATSEP ALL FLOATSEP CARDINAL FLOATSEP CARDINAL FLOATSEP FLOAT END"
                      ,"this.valid=true;this.alt=parseFloat($2); this.lat=[parseFloat($4), $5]; this.lon=[parseFloat($6), $7]; this.speed=parseFloat($8); this.course=parseFloat($9);"],
                ["FULL FLOATSEP ALL FLOATSEP CARDINAL FLOATSEP CARDINAL FLOATSEP INT END"
                      ,"this.valid=true;this.alt=parseFloat($2); this.lat=[parseFloat($4), $5]; this.lon=[parseFloat($6), $7]; this.speed=parseFloat($8); this.course=parseInt($9);"],
                ["FULL FLOATSEP ALL FLOATSEP CARDINAL FLOATSEP CARDINAL FLOATSEP END"
                      ,"this.valid=true;this.alt=parseFloat($2); this.lat=[parseFloat($4), $5]; this.lon=[parseFloat($6), $7]; this.speed=parseFloat($8);"],
    ...
    this.uuid      = "DevAdaptater:" + "TK103/GPS103";
...
DevAdapter.prototype.GpsNormalize =function(data) {
    ...
    if (data.date === undefined) {
        ...
        y='20' + data.date.substring (0,2);
        m=data.date.substring (2,4)-1;  //warning january=0 !!!
        d=data.date.substring (4,6);
        h=data.date.substring (6,8);
        n=data.date.substring (8,10);
 ...
 */
DevAdapter.prototype.SendCommand = function(socket, action, args) {
        switch (action) {   
          case "WELLCOME": 
                // some optional init operations
                break;
          ...
          case "GET_SDCARD":  //**,imei:999999999999999,Q,date;
                ...
                console.log ("**** get sd",this.packet )
                socket.write (this.packet);
                break;
...
DevAdapter.prototype.ParseData = function(device, socket, buffer) {
        ...
        try {
            var data=this.parser.parse(line); // call jison parser
        } catch (err) {
        ...
        switch (data.cmd) {
          ...
          default:   // provide a copy of parsed data to device
            this.GpsNormalize(data);
            break;     
        };
    // on sucess store parsing result into device object
    device.data=data;
    return (0);
};
