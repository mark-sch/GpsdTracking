...
DevAdapter = function(controler) {
    ...
    "bnf": { // WARNING: only one space in between TOKEN ex: "STOP EOF"
        ...
        , 'GPSDATA' : [ // A,47.618544,-2.760980,0.00,201.00,57.00,89,*2c\r\n
                ["ALL FLOATSEP FLOATSEP FLOATSEP FLOATSEP FLOATSEP INTSEP CHKSUM"
                      ,"this.lat=parseFloat($2); this.lon=parseFloat($3); this.speed=parseFloat($4); this.crs=parseFloat($5); this.alt=parseFloat($6); this.battery=parseInt($7)"]
        ]
        , 'IMEI'       : [
                ["INT CHKSUM", "$$=$1"]
        ]
    ...
    this.uuid      = "DevAdaptater:" + "TK103/T55";
...
DevAdapter.prototype.GpsNormalize =function(data) {
    ...
    if (data.date === undefined) {
        ...
        y=data.date.substring (0,4);
        m=data.date.substring (4,6)-1;  //warning january=0 !!!
        d=data.date.substring (6,8);
        h=data.date.substring (8,10);
        n=data.date.substring (10,12);
        s=data.date.substring (12,14);
...
DevAdapter.prototype.TestParser = function(testParser) {
    ...
    for (var test in testParser) {
        ...
        console.log ("  --> Emei:%s Cmd:%s Lat:%s Lon:%s Date:%s Speed:%d Course:%d Altitude:%d", data.imei, data.cmd, data.lat, data.lon, data.date, data.speed, data.crs, data.alt);
    }
};
...
DevAdapter.prototype.SendCommand = function(socket, action, arg1) {
        switch (action) {
            ...
            case "HELP":  // return supported commands by this adapter
                ...
                device.controler.daemon.event.emit ("notice", "HELP", listcmd, this.uuid, socket.uuid);
                break;
            default: 
