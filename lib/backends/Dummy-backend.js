...
var DEMOBASEURL='http://www.sinagot.net/gpsdtracking/';
var FakeVesselBase = {
...
for (mmsi in FakeVesselBase) {
    ...
    vessel.img   = DEMOBASEURL + 'images/' +(vessel.name.replace(/ /g,'-') + "x250.jpg").toLowerCase();
    vessel.url   = DEMOBASEURL + 'devices/'+(vessel.name.replace(/ /g,'-') + ".html").toLowerCase();
}
...
var QJpost  = function (job, callback) {
    ...
    if (job.msgtype === 18) {
    try {
        job.lon = job.lon.toFixed(4);
        job.lat = job.lat.toFixed(4);
        job.sog = job.sog.toFixed(2);
        job.cog = job.cog.toFixed(2);
        } catch(err)  {/* ignore invalid numbers */};
    }
    ...
    switch (command) {
      ...
      case "AUTH_IMEI": 
            ...
            if (FakeVesselBase[device.imei] !== undefined) {
                ...
                device.type = 0;
                device.call = "NoCall";
                device.img  = DEMOBASEURL + "images/unknown-devicex250.jpg";
                device.url  = DEMOBASEURL + "devices/unknown-device.html";
            }
