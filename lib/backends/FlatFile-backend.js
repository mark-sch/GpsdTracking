    ...
    switch (command) {
      ...
      case "AUTH_IMEI": 
        ...
        case "UPDATE_POS" :
            ...
            device.element ++;
            speedms = data.speed * 1852 / 3600; // move from knts to m/s
            
            buffer = "  <trkpt lat=\"" + data.lat         + "\" lon=\"" + data.lon +"\">\n"
                    +"     <speed>"    + speedms                 + "</speed>\n"
                    +"     <course>"   + data.crs         + "</course>\n"
                    +"     <ele>"      + data.alt         + "</ele>\n"
                    +"     <time>"     + new Date().toISOString()+ "</time>\n"
                    +"     <name>trackpts-" + device.element     + "</name>\n"
                    +"     <desc>lat.="+data.lat+",lon.="+data.lon +",Alt.="+data.alt+ "m. Speed=" +speedms+ "m/s. Course="+data.crs+"</desc>\n"
                    +"  </trkpt>\n";
