...
function BackendStorage (gpsd, opts){
    ...
    this.uid="flatfile@" + opts.file.store +'/'+ opts.file.prefix + '*';
...
// Typically would last position of a given device
BackendStorage.prototype.LookupDev = function (callback, devid, args) {
  // let callback application with result
  this.Debug (0,"Hoops FlatFile Backend does not implement LookupDev");
  callback (null);
};

BackendStorage.prototype.Connect = function (gpsd) {
    this.Debug (3,"Connect backend from :%s", gpsd.uid);
...
BackendStorage.prototype.UpdateDev = function (device, command) {
    ...
    switch (command) {
      ...
      case "AUTH_IMEI": 
            ...
            this.Debug (3,"Creating GPRX File for device=%s path=%s", device.uid, filename);
            ...
            try {
                if (device.newfile) {
                    ...
                    fs.writeSync (device.gpxfd,"<author>Fulup (GpsdTracking)</author>\n\n");
                }                
            } catch (err) {          
        ...
        case "UPDATE_POS" :
            ...
            device.element ++;
            speedms = device.stamp.speed * 1852 / 3600; // move from knts to m/s
            
            buffer = "  <trkpt lat=\"" + device.stamp.lat         + "\" lon=\"" + device.stamp.lon +"\">\n"
                    +"     <speed>"    + speedms                 + "</speed>\n"
                    +"     <course>"   + device.stamp.crs         + "</course>\n"
                    +"     <ele>"      + device.stamp.alt         + "</ele>\n"
                    +"     <time>"     + new Date().toISOString()+ "</time>\n"
                    +"     <name>trackpts-" + device.element     + "</name>\n"
                    +"     <desc>lat.="+device.stamp.lat+",lon.="+device.stamp.lon +",Alt.="+device.stamp.alt+ "m. Speed=" +speedms+ "m/s. Course="+device.stamp.crs+"</desc>\n"
                    +"  </trkpt>\n";
       ...
       case "LOGOUT" :
            // if device did not login, quit without writing gpx trailler
            if (device.gpxfd === undefined) return (0);
