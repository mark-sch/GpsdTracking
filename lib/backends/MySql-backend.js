...
function MySql2GpsdPosition (data) {
...
NormalizeDate =function(date) {
    var isodate= date.toISOString().replace(/T/, ' ').replace(/\..+/, '');
    return (isodate);
};
...
BackendStorage.prototype.LookupDev = function (callback, devid, args) {
    var device= this.gpsd.activeClients [devid]; // device.sqlid was updated at authentication
    
    // user was not properly authenticated by MySql
    if (device.sqlid === undefined) return(-1);
    
    var sqlQuery= "Select  latitude,longitude, speed, altitude, course, time from positions "
                  + " WHERE device_id=" + device.sqlid + " ORDER BY TIME DESC LIMIT " + args;
    
    console.log ("sqlQuery=%s", sqlQuery);
    
    this.base.query (sqlQuery , function (err, dbresult, fields) {
        if (err) {
            device.Debug (0,"MySql LookupDev Err=%s",err);
            return (-1);
        }
        var positions=[];
        ...
        for (var idx = 0; (idx < dbresult.length); idx ++) {
            ...
            positions.push (new MySql2GpsdPosition (dbresult[idx]));
        }
        // finally when reponse array is ready callback application
        callback (positions);
    });    
    return (0);
};
    ...
    switch (command) {
      ...
      case "AUTH_IMEI": 
        ...
        case "UPDATE_POS" :
            ...
            if (data.lat ===0 || data.lon === 0) return (-1);
            ...
            var post  = {
                ...
                time        : NormalizeDate(device.stamp.date),  // traccar want date as a UTC string
                valid       : device.stamp.valid, // true if device.stamp is a gps time, wrong other wise
                latitude    : device.stamp.lat,
                longitude   : device.stamp.lon,
                speed       : device.stamp.speed,
                altitude    : device.stamp.alt,
                course      : device.stamp.crs
            };
