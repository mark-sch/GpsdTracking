...
var CreateConnection =function (backend) {
    ...
    backend.Debug (4, "MySQL creating connection [%s]", backend.uuid);
    ...
    backend.base.on('error', function(err) {
        ...
        if(err.code === 'PROTOCOL_CONNECTION_LOST') {
             backend.Debug (4, "MySQL connection lost [%s/%d] automatic connect retry in 10s]", backend.uuid, backend.count);
    ...
    backend.base.connect (function(err) {
        if (err) {
             backend.Debug (4, "MySQL connection fail [%s/%d] automatic connect retry in 10s]", backend.uuid, backend.count);
             setTimeout(function () {CreateConnection (backend);}, MYSQL_RECONNECT_TIMER);  
            
        } else {
            backend.Debug (5,"MySQL Connect Done [%s]",  backend.uuid);
        }
    });
};
...
function BackendStorage (gpsd, opts){
    ...
    this.uuid ="mysql:" + opts.mysql.username + "@" + opts.mysql.hostname + "/" + opts.mysql.basename;
...
BackendStorage.prototype.Query = function (device, command) {
    ...
    this.Debug (6,"Query MySQL device:%s imei=%s Command:%s", device.uuid, device.imei, command);
    ...
    switch (command) {
      ...
      case "AUTH_IMEI": 
            ...
            sqlQuery.on("result", function(result) {
                ...
                // update active device pool [note device.imei is set by DevClient before SQL login]
        ...
        case "UPDATE_POS" :
            ...
            var post  = {
                ...
                time        : NormalizeDate(device.data.date),  // traccar want date as a UTC string
                valid       : device.data.valid, // true if device.data is a gps time, wrong other wise
                latitude    : device.data.lat,
                longitude   : device.data.lon,
                speed       : device.data.speed,
                altitude    : device.data.alt,
                course      : device.data.crs,
                power       : device.data.bat    // percentage of remaining internal battery
            };
      ...
      case "LOGOUT" :   // device quit place it offline
          device.loged = false;
          break;
